...
def get_folder_name():
    ...
    if len(sys.argv) == 2:
        ...
        if not DIR_PATH:
            return print(f"You didn't type a path")
            ...
            try:
                for key, values in EXTENSIONS.items():
                    if str(entry).split(".")[1].lower() in values:
                        os.chdir(path_obj)
                        os.rename(entry.name, translate.normalize(entry.name))
                        names_dict[key].append(translate.normalize(entry.name))
                ...
                if str(entry).split(".")[1].lower() not in known_extension:
                    ...
                    names_dict["unknown"].append(
                        translate.normalize(entry.name))
